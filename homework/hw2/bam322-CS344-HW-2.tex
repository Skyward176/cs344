...
\begin{solution}
  \begin{enumerate}
    ...
    \item Now, run the algorithm on the left half of the array where low = low and high = middle -1 and on the right half with low = middle+1 and high = high.
    \item If A[middle] is not a local minimum, then check if A[middle-1] < A[middle]. If this is the case, then there must be a local minimum in the left half of the array. Otherwise, there must be a local minimum in the right half of the array. Run recursively on the right half.
  \end{enumerate}
  %=================================================
